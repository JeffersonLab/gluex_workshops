...
void genData(){
 ...
 //tt->SetBranchAddress("PipPim_FM_combos", &KpKm_Mass_combos, &b_KpKm_Mass_combos);
  ...
  testsig.CreateComponent("omega",BREITWIGNER,5.3,1, 1.1, .2,false);
  
  testsig.GetParameterFromComponent("omega","omega_Mass")->setVal(1.1);
  ...
  testsig.GetParameterFromComponent("omega","omega_Width")->setVal(.2);
...
LineShape_Library sig("sig", "sig", BValues,PiPlus,PiMinus);
  ...
  sig.GetParameterFromComponent("omega","omega_Mass")->setVal(1.1);
  ...
  sig.GetParameterFromComponent("omega","omega_Width")->setVal(.2);
  ...
  sig.GetSingleComponent_PDF("omega").plotOn(frame_KKMass,RooFit::LineColor(kGreen),RooFit::Normalization(sig.GetIntegral("omega")));
  sig.GetSingleComponent_PDF("rho").plotOn(frame_KKMass,RooFit::LineColor(kRed),RooFit::Normalization(sig.GetIntegral("rho")));
