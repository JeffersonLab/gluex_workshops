...
extern "C"
{
	void InitPlugin(JApplication *locApplication)
	{
		...
		locApplication->AddProcessor(new DEventProcessor_p3pi_PiMinusMiss_P4V()); //register this plugin
		locApplication->AddFactoryGenerator(new DFactoryGenerator_p3pi_PiMinusMiss_P4V()); //register the factory generator
	}
} // "C"
...
// init
//------------------
jerror_t DEventProcessor_p3pi_PiMinusMiss_P4V::init(void)
{
	// This is called once at program startup.

	/*
	//OPTIONAL: Create an EventStore skim.  
	string locSkimFileName = "p3pi_PiMinusMiss_P4V.idxa";
...
// brun
//------------------
jerror_t DEventProcessor_p3pi_PiMinusMiss_P4V::brun(jana::JEventLoop* locEventLoop, int32_t locRunNumber)
{
...
// evnt
//------------------
jerror_t DEventProcessor_p3pi_PiMinusMiss_P4V::evnt(jana::JEventLoop* locEventLoop, uint64_t locEventNumber)
{
	...
	locEventWriterROOT->Fill_DataTrees(locEventLoop, "p3pi_PiMinusMiss_P4V");
	...
	for(size_t loc_i = 0; loc_i < locParticleCombos.size(); ++loc_i)
	{
		...
		if(locParticleCombo->Get_Reaction()->Get_ReactionName() != "p3pi_PiMinusMiss_P4V")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(locAnalysisResults->Get_Reaction()->Get_ReactionName() != "p3pi_PiMinusMiss_P4V")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(!locPassedParticleCombos.empty())
			locEventWriterREST->Write_RESTEvent(locEventLoop, "p3pi_PiMinusMiss_P4V"); //string is part of output file name
	}
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		japp->WriteLock("p3pi_PiMinusMiss_P4V.idxa"); //Lock is unique to this output file
		{
			dEventStoreSkimStream << locRunNumber << " " << locEventNumber << " " << locUniqueID << endl;
		}
		japp->Unlock("p3pi_PiMinusMiss_P4V.idxa");
	}
	*/
...
int DEventProcessor_p3pi_PiMinusMiss_P4V::Get_FileNumber(JEventLoop* locEventLoop) const
{
...
// erun
//------------------
jerror_t DEventProcessor_p3pi_PiMinusMiss_P4V::erun(void)
{
...
// fini
//------------------
jerror_t DEventProcessor_p3pi_PiMinusMiss_P4V::fini(void)
{
