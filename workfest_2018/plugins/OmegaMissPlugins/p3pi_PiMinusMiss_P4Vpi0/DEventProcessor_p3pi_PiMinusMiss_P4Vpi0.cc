...
extern "C"
{
	void InitPlugin(JApplication *locApplication)
	{
		...
		locApplication->AddProcessor(new DEventProcessor_p3pi_PiMinusMiss_P4Vpi0()); //register this plugin
		locApplication->AddFactoryGenerator(new DFactoryGenerator_p3pi_PiMinusMiss_P4Vpi0()); //register the factory generator
	}
} // "C"
...
// init
//------------------
jerror_t DEventProcessor_p3pi_PiMinusMiss_P4Vpi0::init(void)
{
	// This is called once at program startup.

	/*
	//OPTIONAL: Create an EventStore skim.  
	string locSkimFileName = "p3pi_PiMinusMiss_P4Vpi0.idxa";
...
// brun
//------------------
jerror_t DEventProcessor_p3pi_PiMinusMiss_P4Vpi0::brun(jana::JEventLoop* locEventLoop, int32_t locRunNumber)
{
...
// evnt
//------------------
jerror_t DEventProcessor_p3pi_PiMinusMiss_P4Vpi0::evnt(jana::JEventLoop* locEventLoop, uint64_t locEventNumber)
{
	...
	locEventWriterROOT->Fill_DataTrees(locEventLoop, "p3pi_PiMinusMiss_P4Vpi0");
	...
	for(size_t loc_i = 0; loc_i < locParticleCombos.size(); ++loc_i)
	{
		...
		if(locParticleCombo->Get_Reaction()->Get_ReactionName() != "p3pi_PiMinusMiss_P4Vpi0")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(locAnalysisResults->Get_Reaction()->Get_ReactionName() != "p3pi_PiMinusMiss_P4Vpi0")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(!locPassedParticleCombos.empty())
			locEventWriterREST->Write_RESTEvent(locEventLoop, "p3pi_PiMinusMiss_P4Vpi0"); //string is part of output file name
	}
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		japp->WriteLock("p3pi_PiMinusMiss_P4Vpi0.idxa"); //Lock is unique to this output file
		{
			dEventStoreSkimStream << locRunNumber << " " << locEventNumber << " " << locUniqueID << endl;
		}
		japp->Unlock("p3pi_PiMinusMiss_P4Vpi0.idxa");
	}
	*/
...
int DEventProcessor_p3pi_PiMinusMiss_P4Vpi0::Get_FileNumber(JEventLoop* locEventLoop) const
{
...
// erun
//------------------
jerror_t DEventProcessor_p3pi_PiMinusMiss_P4Vpi0::erun(void)
{
...
// fini
//------------------
jerror_t DEventProcessor_p3pi_PiMinusMiss_P4Vpi0::fini(void)
{
