...
extern "C"
{
	void InitPlugin(JApplication *locApplication)
	{
		...
		locApplication->AddProcessor(new DEventProcessor_omega_solutions()); //register this plugin
		locApplication->AddFactoryGenerator(new DFactoryGenerator_omega_solutions()); //register the factory generator
	}
} // "C"
...
// init
//------------------
jerror_t DEventProcessor_omega_solutions::init(void)
{
	// This is called once at program startup.

	/*
	//OPTIONAL: Create an EventStore skim.  
	string locSkimFileName = "omega.idxa";
...
// brun
//------------------
jerror_t DEventProcessor_omega_solutions::brun(jana::JEventLoop* locEventLoop, int32_t locRunNumber)
{
...
// evnt
//------------------
jerror_t DEventProcessor_omega_solutions::evnt(jana::JEventLoop* locEventLoop, uint64_t locEventNumber)
{
	...
	const DEventWriterROOT* locEventWriterROOT = NULL;
	locEventLoop->GetSingle(locEventWriterROOT);
	locEventWriterROOT->Fill_DataTrees(locEventLoop, "omega");
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locParticleCombos.size(); ++loc_i)
	{
		...
		if(locParticleCombo->Get_Reaction()->Get_ReactionName() != "omega")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(locAnalysisResults->Get_Reaction()->Get_ReactionName() != "omega")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		japp->WriteLock("omega.idxa"); //Lock is unique to this output file
		{
			dEventStoreSkimStream << locRunNumber << " " << locEventNumber << " " << locUniqueID << endl;
		}
		japp->Unlock("omega.idxa");
	}
	*/
...
int DEventProcessor_omega_solutions::Get_FileNumber(JEventLoop* locEventLoop) const
{
...
// erun
//------------------
jerror_t DEventProcessor_omega_solutions::erun(void)
{
...
// fini
//------------------
jerror_t DEventProcessor_omega_solutions::fini(void)
{
