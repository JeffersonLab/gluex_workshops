...
\section{detector simulation: hdgeant}
...
\I just say {\tt hdgeant}
\ei
}

\section{addition of detector resolution: mcsmear}

\f{\ft{mcsmear overview}
\bi
\I hit from hdgeant are perfect, e. g., exact deposited energy
\I new information generated and added to the event with appropriate detector resolution introduced
\I original perfect energy saved
\ei
}
...
Usage:
  ...
  options:
    ...
    -u#      Sigma CDC anode drift time in ns (def:0ns)
             (NOTE: this is only used if -y is also specified!)
...
\ft{Exercises}
\bi
\I Generate events with genr8
\I Generate events with bggen
